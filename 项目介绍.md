...
> **最后更新**: 2025年11月11日  
> **定位**: 面向 ECMAScript 5.1 子集的轻量级 JavaScript 前端编译器
...
`js_compiler_by_c` 是一个使用 C 语言实现的 JavaScript 编译器前端，支持词法分析、语法分析、AST 生成与自动分号插入（ASI）。项目遵循 ECMAScript 5.1 规范，提供完整的语法检查能力。
...
- **词法分析器** (`js_lexer.exe`)
  - 识别 27 个 ES5 关键字
  - 支持 73+ 种运算符和分隔符
  - 数字/字符串字面量解析（整数、浮点、科学计数法、十六进制）
  - 单行和多行注释处理
  - 精确的行号/列号跟踪

- **语法分析器** (`js_parser.exe`)
  - 完整的语句支持（if/else、for、while、do-while、switch、try-catch-finally）
  - 表达式解析（算术、逻辑、位运算、三元、逗号）
  - 对象和数组字面量
  - 函数声明和调用
  - AST 构建与 `--dump-ast` 调试输出

- **自动分号插入 (ASI)**
  - 遵循 ECMA-262 11.9 节规范
  - 换行、EOF、受限产生式三类触发
  - 智能控制语句保护
...
# 安装 Make（可选，使用 Makefile 时需要）
pacman -S make
```

#### 第三步：配置 build.bat 路径

克隆项目后，打开 `build.bat`，根据实际安装路径修改以下变量：
...
| 命令                    | 功能                         | 适用平台    |
| ----------------------- | ---------------------------- | ----------- |
| `.\build.bat`           | 构建词法分析器               | Windows     |
| `.\build.bat parser`    | 构建语法分析器               | Windows     |
| `.\build.bat test`      | 构建并测试词法分析器         | Windows     |
| `.\build.bat test-parse`| 构建并运行完整测试套件       | Windows     |
| `.\build.bat clean`     | 清理生成文件                 | Windows     |
| `.\build.bat help`      | 显示帮助信息                 | Windows     |
...
- **关键字**（27个）：var, let, const, function, return, if, else, for, while, do, switch, case, default, break, continue, try, catch, finally, throw, with, typeof, delete, void, new, this, in, instanceof
- **运算符**（73+）：
  - 算术：`+`, `-`, `*`, `/`, `%`, `++`, `--`
  - 比较：`<`, `>`, `<=`, `>=`, `==`, `!=`, `===`, `!==`
  - 逻辑：`&&`, `||`, `!`
...
编辑 `build.bat` 或 `Makefile`，将新测试文件加入 `test-parse` 目标。
...
如需加入自动测试，编辑 `build.bat` 或 `Makefile` 的 `test-parse` 目标。
...
**A**: Windows 用户需确保 `build.bat` 中的路径用引号包裹：
