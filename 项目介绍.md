...
> 运行 `make test …`、`build.bat test-parse` 或 `.uild.bat test <path>` 时，会把所有语法错误写入 `build/parser_error_locations.log`（`文件:行:列:错误`）。`build/test_failures.log` 记录完整 stdout/stderr，便于与 Node/V8 行为做 diff。
